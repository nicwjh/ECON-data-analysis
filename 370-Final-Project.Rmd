...
##### 9b + 9c - Clean zip codes #####
 hcris_dataZipClean=as.data.frame(c()) #make blank data frame
 for(i in unique(hcris_data$prvdr_num)){ #loop through each unique provider number 
  testcase=hcris_data%>%filter(prvdr_num==i) %>%  #select zip code variable from ith provider number
   select(zip_code)
  if(nrow(unique(na.omit(testcase)))==1){  # see if there is only 1 zip code associated with the provider
    hcris_dataZipClean=rbind(hcris_dataZipClean,hcris_data %>%
    filter(prvdr_num==i) %>%
  mutate(zip_code=rep(na.omit(zip_code)[1], sum(prvdr_num==i|prvdr_num!=i)))) #if there is only 1 zip code, replace all NA zip code values for the ith provider with it's unique zip code and append to data frame
    } else{
    hcris_dataZipClean=rbind(hcris_dataZipClean,hcris_data %>%
    filter(prvdr_num==i))  #if there is more than 1 zip code, do nothing but add data to clean data frame so in end you get all original data back with cleaned zips for NAs that can be unambiguously replaced
    }
 }
...
```{r}
# Helper function to pull state codes from zips. Enter zip code, outputs state code
zipz=usa::zipcodes
...
nostatefixed=hcris_data %>% filter(is.na(state))%>% mutate(state=getstate(zipvec = zip_code))  #mutate state var by inputting zips into getstate to get state codes and replace the NAs with the state code from the output of the getstate function
hastate=hcris_data %>% filter(is.na(state)==F) # separated ones that don't need to be fixed
...
hcris_chainMeanNA=mean(hcris_chain$na_count)
hcris_chainMeanNA
#4.191316

hcris_not_chain$na_count <- apply(hcris_not_chain, 1, function(x) sum(is.na(x)))
hcris_not_chainMeanNA=mean(hcris_not_chain$na_count)
hcris_not_chainMeanNA
#5.279683
