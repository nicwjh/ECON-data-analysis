...
#remove all letters and "-" from zip codes and keep only numbers to create one long zip for ones that need to be fixed this wont change correct ones EX: "Y 11746-1498" goes to "117461498"
 ...
 #make all zips numeric removes leading 0s so not sure what to do ex: 018212 becomes 18212 when as.numeric is used

 sum(is.na(hcris_data$zip_code))
 
 hcris_data %>%
    filter(prvdr_num==162500) %>%
  mutate(zip_code=rep(na.omit(zip_code)[1], sum(prvdr_num==162500|prvdr_num!=162500)))

tempzip=hcris_data


  for(i in unique(tempzip$prvdr_num)){
   tempzip %>%
    filter(prvdr_num==i) %>%
  mutate(zip_code=rep(na.omit(zip_code)[1], sum(prvdr_num==i|prvdr_num!=i)))
    rbind(hcris_data,tempzip)
   
  }
#trying to take filled in zips from other years of the same provider number and fill in the years with the NAs
            

# 10

#imprort data set with zip and state codes already in 
zips=usa::zipcodes
#join data sets by zip code so state code is matched with all zips
zips=usa::zipcodes
zips= zips %>%select("zip","state")
#joining two data frames together so all zips have a state code
hcris_data=left_join(hcris_data,zips, by=c("zip_code"="zip")) %>%
  #making renaming the correct state variable
  mutate(state=state.y) %>%
  #dropping the other state vars so only left with correct state 
   select(-state.x, -state.y)

#now only obs with missing states are the ones with missing zips
