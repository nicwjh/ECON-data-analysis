...
Feel free to follow this outline, or just write as you see fit: 

Sample sentence for analysis: As described in the paper, _________ (here, we put a relatively large amount of fluff explaining why we would choose to analyze the variables above based on stuff from the paper.) Based on our ______ (Method, difference in means?) analysis of lab costs for chain and non-chain providers, we found that average lab costs are ________ (higher/lower) for chains than non-chains. As we touched on previously, this could be because of ________ (pick the reason as explained above based on higher or lower, more fluff from paper). 
