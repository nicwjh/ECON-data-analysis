...
Examining Total Cost (Or lab services cost?), Chain vs. Not Chain
```{r}
#THIS CODE IS WRONG BUT I AM GOING TO OFFICE HOURS 9am FRI TO DO THIS WITH ALEX
#omitting NAs in hcris_data
hcris_noNA = na.omit(hcris_data)

hcris_noNA$epo_cost = as.numeric(hcris_noNA$total_costs_hd_labs)

#subset based on chain or not chain
hcris_chain_noNA = subset(hcris_noNA, chain_indicator == "Y")
hcris_not_chain_noNA = subset(hcris_noNA, chain_indicator == "N")

as.numeric(hcris_chain_noNA$total_costs_hd_labs)

#mean() wasn't working for some reason so got mean from this output
t.test(hcris_chain_noNA$total_costs_hd_labs, mu = 1000, alternative = "two.sided")
#mean = 555680.8 

t.test(hcris_not_chain_noNA$total_costs_hd_labs, mu = 555680.8, alternative = "greater")
```
# Analysis Main Points -- Ellis turning this into paragraph
- Assuming this variable captures average lab expenditure for the entire firm, could be higher or lower for chains based on:
  - could be higher because of increased dosage seen in chains as explained in the paper
  - could be lower because chains have the ability to cut costs with larger-scale supply options (not sure if this is exactly the right way to word this, check in paper)

    The paper explains that "large chains... may have lower average costs due to volume discounts for pharmaceuticals as well as centralized clinical laboratories". Based on this quotation, we decided to analyze how costs differ across chains vs. not chains for epo_cost (Epogen variable, drug) and total_costs_hd_labs (lab cost variable).
    As described in the paper, it was found that acquired facilities alter their treatments in ways that increase reimbursements and decrease costs. Acquired facilities and facilities already apart of the chain are able to have an increase in reimbursement from Medicare as they increase the amount of drugs given to each patient. EPOGEN is a drug used to treat anemia and patients EGPOGEN dosage increased 129% when previously independent facilities were acquired by large chains. Chains also decreased the total costs by replacing higher skilled nurses with lower skilled technicians, increased the ratio of patient to employee, and increased the number of patients per dialysis machine. Overall, these changes that previous independent firms made after being acquired by big chains caused a decrease in the overall quality of care that the patients received. Further, chains were also able to decrease costs by having "centralized clinical laboratories" and discounts on large volume pharmaceuticals which can decrease the average costs. Large chains also have the ability to bargain with commercial insurance companies, and the brand recognition of the bigger chains attract more customers allowing for the ability to have discounted prices and a position to bargain, and overall lowering the costs. 
    Observing these changes made after a facility is acquired by a large chain, as well as the position a large chain is in, we decided to analyze the total lab costs of chain facilities versus non-chain facilities. Based on our method of a difference in means test, we found that average lab costs are lower for chains than non-chains. As we touched on previously, this could be because of large chain dialysis facilities having the ability to have centralized laboratories, discounts on pharmaceuticals, attraction of a large customer segement, and the position to bargain with insurance companies. 
    
    

Examining Drug Cost, Chain vs. Not Chain
```{r}
#THIS CODE IS WRONG BUT I AM GOING TO OFFICE HOURS 9am FRI TO DO THIS WITH ALEX
#omitting NAs in hcris_data
hcris_noNA = na.omit(hcris_data)
#make epo_cost numeric
hcris_noNA$epo_cost = as.numeric(hcris_noNA$epo_cost)

#subset based on chain or not chain
hcris_chain_noNA = subset(hcris_noNA, chain_indicator == "Y")
hcris_not_chain_noNA = subset(hcris_noNA, chain_indicator == "N")
#again, make epo_cost numeric
as.numeric(hcris_chain_noNA$epo_cost)

#mean() wasn't working for some reason so got mean from this output
t.test(hcris_chain_noNA$epo_cost, mu = 1000, alternative = "two.sided")
#mean = 555680.8 

t.test(hcris_not_chain_noNA$epo_cost, mu = 555680.8, alternative = "greater")

#look at difference in mean cost
#is epo_cost per unit or total expenditure for the chain?

sample_chain_noNA = sample(hcris_chain_noNA$epo_cost, 379, replace = TRUE)
aov(hcris_chain_noNA$epo_cost ~ sample_chain_noNA)
```
# Analysis Main Points -- Ellis turning this into paragraph
- Assuming this variable captures average Epogen expenditure for the entire firm, could be higher or lower for chains based on:
  - could be higher because of increased dosage seen in chains as explained in the paper
  - could be lower because chains have the ability to cut costs with larger-scale supply options (not sure if this is exactly the right way to word this, check in paper)

    As described in the paper, facilities acquired by big chains increased the amount of dosage of EPOGEN per patient, which would increase the total cost of the drug, as more is being used per patient. "Patients’ EPOGEN doses increase 129% at independent facilities acquired by large chains." After facilities were acquired by large chains, the method of reporting costs became more standardized and this standardization showed that an "acquired facility's use of EPOGEN becomes nearly ... twice as high" compared to before the facility was acquired. This increase in the amount of EPOGEN leads us to believe that the total drug cost for chain facilities would be greater than total drug cost as non-chain facilities. Based on our ______ (Method, difference in means?) analysis of Epogen cost for chain and non-chain providers, we found that average EPOGEN cost is ________ (higher/lower) for chains than non-chains. As we touched on previously, this could be because of the increased dosages of EPOGEN per patient at facilities that acquired by large chainse. 
